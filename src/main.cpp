...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    if (sqrt(pow(ray_direction.dot((ray_origin-sphere_center)),2)-(ray_direction.dot(ray_direction))*((ray_origin-sphere_center).dot((ray_origin-sphere_center))-pow(sphere_radius,2)))>=0){   
        variablet = fmin(((-1*ray_direction.dot((ray_origin-sphere_center))-sqrt(pow((ray_direction.dot((ray_origin-sphere_center))),2)-ray_direction.dot(ray_direction)*(((ray_origin-sphere_center)).dot((ray_origin-sphere_center))-pow(sphere_radius, 2))))/(ray_direction.dot(ray_direction))),((-1*ray_direction.dot((ray_origin-sphere_center))+sqrt(pow((ray_direction.dot((ray_origin-sphere_center))),2)-ray_direction.dot(ray_direction)*(((ray_origin-sphere_center)).dot((ray_origin-sphere_center))-pow(sphere_radius,2))))/(ray_direction.dot(ray_direction))));
        p = ray_origin+ray_direction*variablet;
        N = p - sphere_center;
        //printf("%.7lf\n",variablet);
        return variablet;
    }
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    Vector3d crossproduct=pgram_u.cross(pgram_v); 
    double dotproduct1=pgram_u.dot(pgram_u);
    double dotproduct2=pgram_u.dot(pgram_v);
    double dotproduct3=pgram_v.dot(pgram_v); 
    double matrixsum=(dotproduct1*dotproduct3-dotproduct2*dotproduct2); 
    double variablet=crossproduct.dot(pgram_origin-ray_origin)/crossproduct.dot(ray_direction);
    Vector3d spot=(ray_origin+variablet*ray_direction);
    Vector3d variableright=spot-pgram_origin;
    double dotproduct4=pgram_u.dot(variableright);
    double dotproduct5=pgram_v.dot(variableright);
    double calculation1=(dotproduct3*dotproduct4-dotproduct2*dotproduct5)/matrixsum;
    double calculation2=(-dotproduct2*dotproduct4+dotproduct1*dotproduct5)/matrixsum;
    bool checkk=(0<=calculation1 && calculation1<=1 && 0<=calculation2 && calculation2<=1);
    Vector3d wacked(ray_origin(0), ray_origin(1),6 - ray_on_xy.squaredNorm());
    if (!checkk)
    {
        return -1;
    }
...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position)
{
    ...
    Vector3d p, N;
    int nearest_object_now = find_nearest_object(ray_origin, ray_direction, p, N);

    double routery= sqrt(std::pow((ray_origin(0))-p(0),2)+std::pow((ray_origin(1))-p(1),2)+std::pow((ray_origin(2))-p(2),2));
    double routery2=sqrt(std::pow((ray_origin(0))-light_position(0),2)+std::pow((ray_origin(1))-light_position(1),2)+std::pow((ray_origin(2))-light_position(2),2));
    if (routery<routery2 && nearest_object_now>0 )
    {
        // Return
        return true;
    }
...
// Vector3d castRay( const Vector3d &orig, const Vector3d &dir, const std::vector<std::unique_ptr<Object>> &objects, const std::unique_ptr<DistantLight> &light, const Options &options) { 
//     Vector3d hitColor = options.backgroundColor; 
//     IsectInfo isect; 
//     if (trace(orig, dir, objects, isect)) { 
//         Vector3d hitPoint = orig + dir * isect.tNear; 
//         Vector3d hitNormal; 
//         Vector2d hitTexCoordinates; 
//         isect.hitObject->getSurfaceProperties(hitPoint, dir, isect.index, isect.uv, hitNormal, hitTexCoordinates); 
//         Vector3d L = -light->dir; 
//         IsectInfo isectShad; 
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        p2=p+1e-4*Li;
        bool visibility=is_light_visible(p2, Li, light_position);
        if(visibility==true){
            continue;
        }
        ...
        const Vector3d Li2= (camera_position - p).normalized();
        Vector3d calculation = (Li+Li2)/(Li+Li2).norm();
        

        // Specular contribution, use obj_specular_color
        specular= obj_specular_color * std::pow(std::max(calculation.dot(N), 0.0), obj_specular_exponent);
    ...
    // use refl_color
    // Vector3d D2=ray_direction.normalized();
    // Vector3d D1=2*(D2.dot(N));
    // Vector3d R=D2-D1.dot(N);
    // R=R.normalized();
    //Vector3d R=(D - 2 * D.dot(N) * N).normalized()
    Vector3d R=(ray_direction.normalized()-2*ray_direction.normalized().dot(N)*N).normalized();
    Vector4d reflection_color(0,0,0,0);
    if(max_bounce>0){
        Vector4d shooter=shoot_ray(p+R*1e-4 , R, max_bounce-1);
        reflection_color=reflection_color+refl_color.cwiseProduct(shooter);
    }
